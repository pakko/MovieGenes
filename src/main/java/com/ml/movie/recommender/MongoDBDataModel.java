 ...
 * 
 * <p>
 * The timestamps (mongoTimestampAdd, mongoTimestampDelete), if present, are
 * assumed to be parseable as a {@code long} or {@link Date}. To express
 * timestamps as {@link Date}s, a {@link DateFormat} must be provided in the
 * class constructor. The default Date format is
 ...
 * {@link #refresh(Collection)} is called. MongoDBDataModel keeps the timestamp
 * of the last update. This variable and the fields mongoTimestampAdd and
 * mongoTimestampDelete help the model to determine if the triple (user, item,
...
public final class MongoDBDataModel implements DataModel {
	...
	private static final String DEFAULT_MONGO_TIMESTAMP_A = "created_at";
	private static final String DEFAULT_MONGO_TIMESTAMP_D = "deleted_at";
	...
	private String mongoTimestampAdd = DEFAULT_MONGO_TIMESTAMP_A;
	private String mongoTimestampDelete = DEFAULT_MONGO_TIMESTAMP_D;
	...
	/**
	 * Creates a new MongoDBDataModel with MongoDB basic configuration (without
	 * authentication)
	 * 
	 * @param host
	 *            MongoDB host.
	 ...
	 *            MongoDB database. If false, the model adds the
	 *            mongoTimestampDelete field with the current date to the
	 *            "deleted" user/item.
	 * @param format
	 ...
	 */
	public MongoDBDataModel(String host, int port, String database,
	 ...
	 * @throws UnknownHostException
	 *             if the database host cannot be resolved
	 * @see #MongoDBDataModel(String, int, String, String, boolean, boolean,
	 *      DateFormat)
	 */
	public MongoDBDataModel(String host, int port, String database,
			String collection, boolean manage, boolean finalRemove,
			DateFormat format, String userIDField, String itemIDField,
			String preferenceField, String timestampField) throws UnknownHostException {
		mongoHost = host;
		mongoPort = port;
		mongoDB = database;
		mongoCollection = collection;
		...
		mongoTimestampAdd = timestampField;
		this.reloadLock = new ReentrantLock();
		buildModel();
	}

	/**
	 * Creates a new MongoDBDataModel with MongoDB basic configuration (with
	 * authentication)
	 * 
	 * @param user
	...
	/**
	 * Creates a new MongoDBDataModel with MongoDB advanced configuration (with
	 * authentication)
	 * 
	 * @throws UnknownHostException
	 *             if the database host cannot be resolved
	...
	public void refresh(Collection<Refreshable> alreadyRefreshed) {
		...
		while (cursor.hasNext()) {
			...
			if (ts.compareTo(date) < 0) {
				ts = date;
			}
		}
		query = new BasicDBObject();
		query.put(mongoTimestampAdd, new BasicDBObject("$gt", mongoTimestamp));
		cursor = collection.find(query);
		while (cursor.hasNext()) {
			Map<String, Object> user = (Map<String, Object>) cursor.next().toMap();
			if (!user.containsKey(mongoTimestampDelete)) {
				String userID = getID(user.get(mongoUserID), true);
				String itemID = getID(user.get(mongoItemID), false);
				float ratingValue = getPreference(user.get(mongoPreference));
				Date date = getDate(user.get(mongoTimestampAdd));
	 ...
	 */
	public boolean isIDInModel(String ID, boolean isUser) {
		DBObject objectIdLong = null;
		if (isUser) {
			objectIdLong = collection.findOne(new BasicDBObject(mongoUserID, ID));
		} else {
			objectIdLong = collection.findOne(new BasicDBObject(mongoItemID, ID));
		}
	...
	private void buildModel() throws UnknownHostException {
		...
		if (!mongoAuth
			...
			// add this to avoid timeout
			cursor.addOption(Bytes.QUERYOPTION_NOTIMEOUT);

			while (cursor.hasNext()) {
				Map<String, Object> user = (Map<String, Object>) cursor.next().toMap();
				if (!user.containsKey(mongoTimestampDelete)) {
					long userID = Long.parseLong(getID(user.get(mongoUserID), true));
					...
					if (user.containsKey(mongoTimestampAdd)
							&& mongoTimestamp.compareTo(getDate(user.get(mongoTimestampAdd))) < 0) {
						mongoTimestamp = getDate(user.get(mongoTimestampAdd));
					}
				}
			}
		}
	...
	private void removeMongoUserItem(String userID, String itemID) {
		if (isUserItemInDB(userID, itemID)) {
			...
			BasicDBObject query = new BasicDBObject();
			query.put(mongoUserID, Long.parseLong(userID));
			query.put(mongoItemID, Long.parseLong(itemID));
			
			if (mongoFinalRemove) {
				log.info(collection.remove(query).toString());
			} else {
				BasicDBObject update = new BasicDBObject();
				update.put("$set", new BasicDBObject(mongoTimestampDelete,
						mongoTimestamp));
	...
	private Date getDate(Object date) {
		...
		if (date.getClass().getName().contains("NumberLong")) {
	...
	private void checkData(String userID, Iterable<List<String>> items,
		...
		if (!add && !isIDInModel(userID, true)) {
			throw new NoSuchUserException();
		}
		for (List<String> item : items) {
			if (!add && !isIDInModel(item.get(0), false)) {
